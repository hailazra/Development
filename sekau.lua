-- 🌀 Auto Unlimited Eggs + Team C Auto Sell (WindUI) – FULL FIX
-- Flow: Team A (place) → Team B (delay, hatch) → Team C (delay, sell) → Team A
-- Fix: Auto Place pakai builder area/grid versi stabil + watchdog + auto pick egg tool
-- Sell rules: dari ReplicatedStorage.Data.PetRegistry.PetList (bukan Backpack); favorit auto-skip
...
-- World roots
local FARM_ROOT = workspace:WaitForChild("Farm"):WaitForChild("Farm")
local IMPORTANT = FARM_ROOT:WaitForChild("Important")
local OBJ_ROOT  = IMPORTANT:WaitForChild("Objects_Physical")
local PLANT_LOC = IMPORTANT:FindFirstChild("Plant_Locations")
...
-- Auto Sell rules (shared dengan script Auto Sell Pet terpisah kalau ada)
...
--==============================
-- Place Eggs (grid + MaxSlots) – STABLE BUILDER (V1)
--==============================
local Areas = {}
...
local function placeAt(point)
    if not SelectedEggTool or not SelectedEggTool.Parent then
        local eggs=listEggs(); SelectedEggTool=eggs[1]
        if not SelectedEggTool then return false,"No egg tool" end
    end
    if not ensureEquipped(SelectedEggTool) then return false,"Equip fail" end
...
task.spawn(function()
    ...
    -- Watchdog: kalau grid kosong (Plant_Locations belum ke-load), coba rebuild periodik
    task.spawn(function()
        while true do
            if #grid == 0 then
                rebuildAreas()
    ...
    while true do
        if not CFG.Run then
            ...
            if MODE=="A" then
                ...
                -- Pastikan egg tool ada
                if not SelectedEggTool or not SelectedEggTool.Parent then
                    local eggs = listEggs()
                    SelectedEggTool = eggs[1]
                    if not SelectedEggTool then
                        setStatus("Team A • No egg tool in Backpack")
                    end
                end
                ...
                if #grid == 0 then
                    ...
                    if #grid == 0 then
                        setStatus("Team A • No Plant_Locations area found")
                    end
                end

                if CFG.AutoPlace and CFG.MaxSlots>0 and #grid>0 and SelectedEggTool then
                    ...
                    setStatus(("Team A • areas=%d grid=%d eggs=%d • ready %d/%d")
                        :format(#Areas, #grid, #listEggs(), select(2,allEggsReady()), select(3,allEggsReady())))
                else
...
local Window = WindUI:CreateWindow({
    ...
    Folder = "AutoUE",
    Size = UDim2.fromOffset(250, 250), -- tinggi dilebihin biar Active Rules muat 5+ pet
    Theme = "Dark",
    SideBarWidth = 200,
})
...
T.PlaceTab:Slider({
    ...
    Callback = function(val) CFG.GridStep = tonumber(val) or CFG.GridStep end
})
...
local function refreshEggDropdown()
    ...
    EggDropdown = T.PlaceTab:Dropdown({
        ...
        Callback = function(name)
            for _,t in ipairs(listEggs()) do if t.Name==name then SelectedEggTool=t; break end end
        end
    })
    SelectedEggTool = listEggs()[1]
end
refreshEggDropdown()
